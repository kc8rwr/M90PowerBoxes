...
module power_box(size=[55, 54, 55], step=[40, 27], bolt_size = "m4", back_lip = 5, top_cable_d, top_cable_bolt, bottom_cable_d, bottom_cable_bolt, wall=2){
  ...
  let(washer_v = unf_wsh_v(bolt_size),
      ...
      c14_zpos = wall,
      c14_fuse_spacing = wall,
      fuse_holder_zpos = c14_zpos + unf_c14_size().x + c14_fuse_spacing,
	...
	difference(){
	  ...
	  //Fuse Holder
	  translate([wall, back_size.y/2, fuse_holder_zpos+(fuse_holder_outer_diameter/2)]){
	...
	color(support_color, alpha=0.5){
	  ...
	  translate([back_size.x-back_lip-wall, back_size.y-support_skin, 0]){
	    cube([step.x-tab_length, support_skin, front_size.z-max(wall, back_lip)]);
	  }
	}
      }
    }
